...
//  MatchingPairsTest
//
//  Created by Andrew Smith on 19/04/2016.
...
@property (strong, nonatomic) SKNode *Card1_L3;
@property (strong, nonatomic) SKNode *Card2_L3;
...
@implementation GameSceneLevel3
{
    ...
    SKSpriteNode *background_L3;
    SKSpriteNode *healthMeter_L3;
    SKSpriteNode *nextLevelButton_L3;
    SKSpriteNode *gameOverButton_L3;
    SKLabelNode *scoreLabel_L3;
    SKLabelNode *clefLabel_L3;
    SKLabelNode *levelLabel_L3;
    SKLabelNode *gameOverLabel_L3;
    SKLabelNode *levelCompleteLabel_L3;
    SKTexture *FullHealthMeter4;
    SKTexture *HealthMeter4m1;
    SKTexture *HealthMeter4m2;
    SKTexture *HealthMeter4m3;
    SKTexture *EmptyHealthMeter4;
    SKSpriteNode *medal;
    SKTexture *gold;
    SKTexture *silver;
    SKTexture *bronze;
...
int grid_L3[9]; // used to store card in each grid position for musical stave cards
int grid2_L3[9]; // used to store card in each grid position for speaker note cards
int xGridSize_L3 = 160; // used to size grid x axis
int yGridSize_L3 = 200; // used to size grid y axis
int currentScore_L3 = 0;
int correctMatches_L3 = 0;
int scoreMultiplier_L3 = 1;
int healthCount_L3 = 4;
BOOL selectedCard_L3 = NO;
...
-(void)didMoveToView:(SKView *)view
{
    ...
    {
        case 0:
            [self randomCards1_L3];
            break;
        case 1:
            [self randomCards2_L3];
            break;
        case 2:
            [self randomCards3_L3];
            break;
    }
}
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    for (UITouch *touch in touches)
    {
        ...
        if ((![node.name isEqualToString:@"other"])) // if the node touched corresponds to a card sprite node...
        {
            ...
            if (selectedCard_L3 == NO) // if first card is touched...
            {
                selectedCard_L3 = YES; // set state to yes
                _Card1_L3 = node; // save the card's state
                
                [_Card1_L3 runAction:highlight]; // run action to highlight the card
                NSLog(@"Name of first card is %@", _Card1_L3.name);
                
                // Call method to play the corresponding note audio when the card is pressed
                [self playSoundsForCardWithName:_Card1_L3.name];
            }
            
            else if (selectedCard_L3 == YES) // if second card is touched...
            {
                _Card2_L3 = node; // save the card's state
                NSLog(@"Name of second card is %@", _Card2_L3.name);
                
                [_Card2_L3 runAction:highlight];
                
                if (_Card1_L3.name == _Card2_L3.name) // if names match, then the match is successful...
                {
                    
                    if (_Card2_L3 == _Card1_L3) // ...but if the second touch is the same as the first...
                    {
                        [_Card1_L3 runAction: unhighlight];
                    }
                    
                    else if (_Card1_L3.zPosition == _Card2_L3.zPosition) // ...or if both cards are the same type...
                    {
                        [_Card1_L3 runAction: unhighlight];
                        [_Card2_L3 runAction: unhighlight];
                        [self lifeMeter_L3]; // lose a heart, so call life meter method
                    }
                    ...
                    else
                    {
                        ...
                        NSLog(@"Correct match!");
                        [_Card1_L3 runAction:successfulMatch];
                        [_Card2_L3 runAction:successfulMatch];
                        
                        correctMatches_L3 = correctMatches_L3 + 1;
                        
                        // Handle scoring
                        
                        currentScore_L3 = (currentScore_L3 + (1 * scoreMultiplier_L3)); // add points
                        
                        scoreMultiplier_L3 = scoreMultiplier_L3 + 1; // every correct answer adds 1 to multiplier
                        
                        scoreLabel_L3.text = [NSString stringWithFormat:@"Score: %d", currentScore_L3]; // update label
                    }
                }
                ...
                selectedCard_L3 = NO; // Reset the state of selectedCard
                NSLog(@"selectedCard state is = %hhd", selectedCard_L3);
                
                // Call method to play the corresponding note audio when the card is pressed
                [self playSoundsForCardWithName:_Card2_L3.name];
                
            }
        }
        
        // Call levelComplete method
        [self levelComplete_L3];
...
{
    // Set up background
    background_L3 = [SKSpriteNode spriteNodeWithImageNamed:@"Background.png"];
    background_L3.name = @"other";
    background_L3.size = self.frame.size;
    background_L3.position = CGPointMake(CGRectGetMidX(self.frame), CGRectGetMidY(self.frame));
    self.scaleMode = SKSceneScaleModeAspectFit;
    background_L3.zPosition = -2; // keep the background behind everything else
    
    [self addChild:background_L3];
    
    // Set up score label
    scoreLabel_L3 = [SKLabelNode labelNodeWithFontNamed:@"DevanagariSangamMN"];
    scoreLabel_L3.text = @"Score: 0";
    scoreLabel_L3.fontSize = 30;
    scoreLabel_L3.position = CGPointMake(CGRectGetMidX(self.frame)+400, CGRectGetMidY(self.frame)+280);
    scoreLabel_L3.name = @"other";

    [self addChild:scoreLabel_L3];
    
    // Set up clef label
    clefLabel_L3 = [SKLabelNode labelNodeWithFontNamed:@"DevanagariSangamMN"];
    clefLabel_L3.text = @"Clef: Treble";
    clefLabel_L3.fontSize = 30;
    clefLabel_L3.position = CGPointMake(CGRectGetMidX(self.frame)-390, CGRectGetMidY(self.frame)+330);
    clefLabel_L3.name = @"other";
    
    [self addChild:clefLabel_L3];
    
    // Set up level label
    levelLabel_L3 = [SKLabelNode labelNodeWithFontNamed:@"DevanagariSangamMN"];
    levelLabel_L3.text = @"Level: 3";
    levelLabel_L3.fontSize = 30;
    levelLabel_L3.position = CGPointMake(CGRectGetMidX(self.frame)+400, CGRectGetMidY(self.frame)+330);
    levelLabel_L3.name = @"other";
    
    [self addChild:levelLabel_L3];
    
    // Set up game over label
    gameOverLabel_L3 = [SKLabelNode labelNodeWithFontNamed:@"DevanagariSangamMN"];
    gameOverLabel_L3.text = @"Game Over!";
    gameOverLabel_L3.fontSize = 100;
    gameOverLabel_L3.position = CGPointMake(CGRectGetMidX(self.frame), CGRectGetMidY(self.frame));
    gameOverLabel_L3.name = @"other";
    gameOverLabel_L3.zPosition = 3;
    gameOverLabel_L3.alpha = 0;
    
    [self addChild:gameOverLabel_L3];
    
    // Set up "next level" button
    nextLevelButton_L3 = [SKSpriteNode spriteNodeWithImageNamed:@"ContinueButton"];
    nextLevelButton_L3.yScale = 0.5;
    nextLevelButton_L3.xScale = 0.5;
    nextLevelButton_L3.position = CGPointMake(CGRectGetMidX(self.frame), CGRectGetMidY(self.frame)-240);
    nextLevelButton_L3.name = @"nextLevel";
    nextLevelButton_L3.zPosition = 3;
    nextLevelButton_L3.alpha = 0;
    
    [self addChild:nextLevelButton_L3];
    
    // Set up "game over" button
    gameOverButton_L3 = [SKSpriteNode spriteNodeWithImageNamed:@"ContinueButton"];
    gameOverButton_L3.yScale = 0.5;
    gameOverButton_L3.xScale = 0.5;
    gameOverButton_L3.position = CGPointMake(CGRectGetMidX(self.frame), CGRectGetMidY(self.frame)-240);
    gameOverButton_L3.name = @"gameOverButton";
    gameOverButton_L3.zPosition = 3;
    gameOverButton_L3.alpha = 0;
    
    [self addChild:gameOverButton_L3];
    
    // Set up "level complete" label
    levelCompleteLabel_L3 = [SKLabelNode labelNodeWithFontNamed:@"DevanagariSangamMN"];
    levelCompleteLabel_L3.text = @"Level Complete!";
    levelCompleteLabel_L3.fontSize = 100;
    levelCompleteLabel_L3.position = CGPointMake(CGRectGetMidX(self.frame), CGRectGetMidY(self.frame)+120);
    levelCompleteLabel_L3.name = @"other";
    levelCompleteLabel_L3.zPosition = 3;
    levelCompleteLabel_L3.alpha = 0;
    
    [self addChild:levelCompleteLabel_L3];
    ...
    FullHealthMeter4 = [SKTexture textureWithImageNamed:@"FullHealthMeter4"];
    HealthMeter4m1 = [SKTexture textureWithImageNamed:@"HealthMeter4-1"];
    HealthMeter4m2 = [SKTexture textureWithImageNamed:@"HealthMeter4-2"];
    HealthMeter4m3 = [SKTexture textureWithImageNamed:@"HealthMeter4-3"];
    EmptyHealthMeter4 = [SKTexture textureWithImageNamed:@"EmptyHealthMeter4"];
    
    healthMeter_L3 = [SKSpriteNode spriteNodeWithTexture:FullHealthMeter4];
    healthMeter_L3.xScale = 0.5;
    healthMeter_L3.yScale = 0.5;
    healthMeter_L3.anchorPoint = CGPointMake(0, 0);
    healthMeter_L3.position = CGPointMake(CGRectGetMidX(self.frame)-470, CGRectGetMidY(self.frame)+255);
    healthMeter_L3.name = @"other";
    
    
    [self addChild:healthMeter_L3];
} // end method
...
-(void)lifeMeter_L3
{
    ...
    SKAction *removeHeart1 = [SKAction setTexture:HealthMeter4m1];
    SKAction *removeHeart2 = [SKAction setTexture:HealthMeter4m2];
    SKAction *removeHeart3 = [SKAction setTexture:HealthMeter4m3];
    SKAction *emptyHearts = [SKAction setTexture:EmptyHealthMeter4];
    ...
    // Handle health meter
    healthCount_L3 = healthCount_L3 - 1; // lose a heart from health meter for incorrect match
    
    if (healthCount_L3 == 3) // lose a heart
    {
        [healthMeter_L3 runAction:removeHeart1];
    }
    
    else if(healthCount_L3 == 2) // lose a heart
    {
        [healthMeter_L3 runAction:removeHeart2];
    }
    
    else if(healthCount_L3 == 1) // lose a heart
    {
        [healthMeter_L3 runAction:removeHeart3];
    }
    
    else if(healthCount_L3 == 0) // game over!
    {
        [healthMeter_L3 runAction:emptyHearts];
        [gameOverLabel_L3 runAction:fadeIn];
        [gameOverButton_L3 runAction:fadeIn];
...
-(void)levelComplete_L3
{
    ...
    if (correctMatches_L3 == 9) // when all nine matches are made, the level is complete.
    {
        [levelCompleteLabel_L3 runAction:fadeIn];
        [nextLevelButton_L3 runAction:fadeIn];
        
        // award a medal.
        
        if (currentScore_L3 == 45 && healthCount_L3 == 4) // highest possible score and max. hearts awards best medal
        {
            [medal runAction:platinumTexture];
        }
        else if (healthCount_L3 == 3)
        {
            [medal runAction:goldTexture];
        }
        else if (healthCount_L3 == 2)
        {
...
-(void)randomCards1_L3
{
    ...
    for (j = 0; j < 3; j++)
    {
        for (i = 0; i < 3; i ++)
        {
            ...
            grid_L3[j*3+i] = randomCard; // store the cards in the grid
            
            staveCard.xScale = 0.5;
            staveCard.yScale = 0.5;
            staveCard.position = CGPointMake(((i*xGridSize_L3)+112),((j*yGridSize_L3)+120));
    ...
    for (j = 0; j < 3; j++)
    {
        for (i = 0; i < 3; i ++)
        {
            ...
            grid2_L3[j*3+i] = randomSpeakerCard; // store the cards in the grid
            
            speakerCard.xScale = 0.5;
            speakerCard.yScale = 0.5;
            speakerCard.position = CGPointMake(((i*xGridSize_L3)+600),((j*yGridSize_L3)+120));
